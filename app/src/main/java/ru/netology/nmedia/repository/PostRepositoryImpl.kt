...
class PostRepositoryImpl @Inject constructor(
    ...
    override var data : Flow<PagingData<FeedFragment.Post>> = Pager(
    ...
    override suspend fun refresh() {
        try {
            val firstId = dao.getFirstId()
            val response = if (firstId != null) {
                apiService.getAfter(firstId, 10)
            } else {
                apiService.getLatest(10)
            }
            if (!response.isSuccessful) {
                throw ApiError(response.code(), response.message())
            }
